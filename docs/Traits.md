...
> **Known issue:** as of the current version, `scopeForSite()` and the auto-fill-on-create logic call `SiteContext::id()` / `SiteContext::has()`, but `SiteContext` only exposes `site(): ?Site` — those two call sites will throw `Error: Call to undefined method`. `yanmar-extranet`'s `Product` and `Event` both use this trait today, but neither has surfaced the bug: both repositories set `$resourceModel->site_id` explicitly before create (from `SiteContext::site()?->id`), so `empty($model->site_id)` is always `false` and the `&&`-short-circuited `has()` call is never reached; `scopeForSite()` itself isn't called anywhere in that codebase either. Any consumer relying on the trait's *own* auto-fill (no `site_id` set beforehand) or calling `scopeForSite()` directly will hit this. Needs a fix in the package (either add `id()`/`has()` to `SiteContext`, or change the trait to use `site()?->id` / `site() !== null` like the rest of the trait already does).
